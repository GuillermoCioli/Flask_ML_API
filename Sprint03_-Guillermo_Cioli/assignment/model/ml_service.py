...
def classify_process():
    ...
    while True:
        ...
        #  ML model on the given data has to run now
        q = json.loads(q.decode("utf-8"))
        class_name, pred_probability = predict(q["image_name"])

        #  Model prediction stored in a dict
        pred = {
            "prediction": class_name,
            "score": round(float(pred_probability), 4),
        }

        #  Store the results on Redis using the original job ID
        job_id = q["id"]
